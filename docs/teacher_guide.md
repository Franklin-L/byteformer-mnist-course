...
3. 使用完整基线命令运行 3 轮，保存终端日志、环境版本、代码提交号和全部结果文件。
4. 使用相同环境运行 5 轮对比实验，另存到 `outputs/epochs5`。检查最佳模型由验证集选择，测试集只用于最终评估。
5. 把实测结果填入课程 PPT，标注设备、样本数、epoch 数和实验日期。若只演示了子集实验，不展示成完整 MNIST 基准成绩。
...
| 运行基线 | 20 分钟 | 解释 epoch、batch、loss、accuracy | 运行 3 轮命令，记录日志 |
| 查看输出 | 15 分钟 | 区分训练、验证、测试 | 打开曲线，读取指标，单图预测 |
| 修改一个参数 | 20 分钟 | 说明公平比较与验证集作用 | 运行 5 轮实验，填写对照表 |
...
主入口为仓库中的 `course_kaggle.ipynb`。学生导入后开启 GPU 与 Internet，代码克隆到 `/kaggle/working/`，依次运行。轻量 `requirements.txt` 保留预装 PyTorch。不要让学生把 `requirements-reproduce.txt` 安装进托管环境。
...
- 说明 MNIST 训练、验证、测试来自不同样本；使用的是固定小样本子集。复现实验需要记录样本数。
- 强调 `best.pt` 按验证集选择。即使第 5 轮训练损失更低，最佳模型也可能来自更早的轮次。
- 对比实验每次从同一官方预训练权重开始，保持默认随机种子，只改 epoch。不要误以为是接着上次的最佳模型继续训练。
- 课前确定比较 3 轮和 5 轮。学生可以报告两组最终测试结果，但不应再根据测试成绩反复改参数。
...
不设置“准确率越高分数越高”的机械规则。设备受限、预先批准只完成 CPU 路线的学生，应采用事先说明的替代作业或评分标准；不要事后将未运行的完整实验按已完成处理。
...
| GPU / CPU | NVIDIA GeForce RTX 4090；CPU 4 线程 |
| 官方预训练来源与文件校验 | Apple 官方 JPEG 预训练 Tiny，SHA256 见 `prepare.py` 和指标记录 |
| 随机种子与切分规则 | 种子42；官方训练集固定分50,000训练池+10,000验证池，再取6,000/1,000；测试集种子43取1,000 |
| 基线：最佳验证准确率 / 轮次 | 88.5%，第3轮 |
| 基线：最终测试准确率 | 88.6%（886/1,000）；独立全测试集评估88.72%（8,872/10,000） |
| 基线运行耗时 | 39.48秒，不含安装下载 |
| 5 轮：最佳验证准确率 / 轮次 | 90.4%，第5轮 |
| 5 轮：最终测试准确率 | 89.5%（895/1,000） |
| 5 轮运行耗时 | 57.70秒，不含安装下载 |
| CPU 快速路线是否实际完成 | 已完成；1轮1,000/200/200，测试20.5%，65.70秒，仅验证流程 |
